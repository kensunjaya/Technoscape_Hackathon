    ...
    <div className="w-screen h-screen flex justify-center items-center bg-background font-sans">
      <form className="form-content p-10 min-w-[40vh] bg-bluepale rounded-xl bg-blueform">
        ...
        <div className="form-group pb-5 w-full">
          <input type="username" id="formUsername" placeholder="username" className="p-3 rounded-xl bg-bluefield text-white w-full" value={username} onChange={(e) => setUsername(e.target.value)} />
        </div>
        <div className="form-group pb-5 w-full">
        ...
        <div className="form-group pb-5 min-w-[20rem]">
          <input type="password" id="formPassword" placeholder="confirm password" className="p-3 rounded-xl bg-bluefield text-white w-full" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} />
        </div>

        {errorMsg !== "" && <div className="text-red-400 mb-5">{errorMsg}</div>}
        <div>
          <button onClick={createAccount} type="button" className="btn btn-primary rounded-xl mr-5 bg-graysbtn text-blueform font-semibold w-full">
            Register
          </button>
          <div className="flex pt-3 justify-end">
            <div className="pr-2 text-sm">Already have an account?</div>
            <Link to="/signin" className="text-white text-sm underline">Login here</Link>
          </div>
        </div>
      </form>
    </div>
